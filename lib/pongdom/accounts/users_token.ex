defmodule Pongdom.Accounts.UsersToken do
  use Ecto.Schema
  import Ecto.Query
  ...
  schema "users_tokens" do
    ...
    belongs_to :users, Pongdom.Accounts.Users
  ...
  def build_session_token(users) do
    ...
    {token, %Pongdom.Accounts.UsersToken{token: token, context: "session", users_id: users.id}}
  end
  ...
  def token_and_context_query(token, context) do
    from Pongdom.Accounts.UsersToken, where: [token: ^token, context: ^context]
  end
  ...
  def users_and_contexts_query(users, :all) do
    from t in Pongdom.Accounts.UsersToken, where: t.users_id == ^users.id
  end

  def users_and_contexts_query(users, [_ | _] = contexts) do
    from t in Pongdom.Accounts.UsersToken, where: t.users_id == ^users.id and t.context in ^contexts
  end
end
